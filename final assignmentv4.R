...
frqt<-findFreqTerms(filedtm, lowfreq = 500, highfreq=Inf)
frqt
...
names(bivar2final)<-c("prefix", "word", "frequency")
...
names(trivar2final)<-c("prefix", "word", "frequency")
...
save(bivar2final, file="bigramd.RData")
save(trivar2final, file="trigramd.RData")
save(fourvar2final, file="fourgramd.RData")
...
if(length(inputspl)==1)
{
inputunigram<-as.data.frame(inputspl[,length(inputspl)])
patternm<-as.character(inputunigram[1,])
match4g<-grep(patternm,bivar2final$prefix)

nextword<-as.data.frame(bivar2final[match4g[1],2])
}

if(length(inputspl)==2){
inputbigram<-as.data.frame(paste(inputspl[,length(inputspl)-1], inputspl[,length(inputspl)],sep=" "))
patternm<-as.character(inputbigram[1,])
match4g<-grep(patternm,trivar2final$prefix)
nextword<-as.character(trivar2final[match4g[1],2])
}
...
nextword<-as.data.frame(fourvar2final[match4g[1],2])
...
#   inputd<-c("dsfdf dfdsf dfsf dddd tool nicd")
#   inputdspl<-as.data.frame(strsplit(inputd, " "))
#   
#   inputspl<-as.data.frame(do.call(rbind,strsplit(inputd, " ")))
#   inputunigram<-as.data.frame(inputspl[,length(inputspl)])
  ...
  nextword<-bivar2final[match4g[1],2]
  
}

if(is.na(nextword)){
  rnword<-length(frqt)
  rnnum<-sample(1:rnword,1)
  nextword<-frqt[rnnum]
